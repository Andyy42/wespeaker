...
# gpus=
num_avg=2
# checkpoint=exp_naki/NAKI-2025-04-10-WavLM-BasePlus-MHFA-emb256-3s-LRS10-Epoch20-no-margin/models/model_3.pt
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for dset in $dataset_path ; do
    if [ $data_type == "shard" ]; then
      python tools/make_shard_list.py --num_utts_per_shard 200 \
          --num_threads 64 \
          --prefix shards \
          --shuffle \
